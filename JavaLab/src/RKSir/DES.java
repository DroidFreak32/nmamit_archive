...
public class DES {
    ...
    private static int feistel(int r, /* 48 bits */ long subkey) {
        ...
        long e = E(r);
        // 2. key mixing
        System.out.println("XOR OPERATION: "+Long.toString(e)+" XOR "+Long.toString(subkey));
        long x = e ^ subkey;
        // 3. substitution
        int dst = 0;
        for (int i=0; i<8; i++) {
    ...
    public static byte[] encrypt(byte[] message, byte[] key) {
        ...
        //for (int i=0; i<message.length; i+=8)
            encryptBlock(message, 0, ciphertext, 0, key);
     ...
     * the same key.
     * 
     * The provided password is converted into a key.
     */
    public static byte[] encrypt(byte[] challenge, String password) {
        byte[] passByte = password.getBytes(Charset.forName("UTF-8"));
        return encrypt(challenge, passByte);
    }
    ...
    public static boolean test(byte[] message, byte[] expected, String password) {
        ...
        byte[] passByte = password.getBytes(Charset.forName("UTF-8"));
        return test(message, expected, passByte);
    }

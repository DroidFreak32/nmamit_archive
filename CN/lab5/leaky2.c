...
printf("\n\nIncomming packet size (%d) is Greater than bucketcapacity-PACKET REJECTED",packet_sz[i]);
else
...
{
if(p_sz_rm <= o_rate)
printf("\n Packet of size %dTransmitted",p_sz_rm),p_sz_rm=0;
else
printf("\n Packet of size %d Transmitted",o_rate),p_sz_rm -= o_rate;
